...
def add_sn_prefix(serial):
    """
    20USBHX2003825   -> SN20USBHX2003825
    SN20USBHX2003825 -> SN20USBHX2003825
    """
...
def remove_sn_prefix(serial):
    """
    SN20USBHX2003825 -> 20USBHX2003825
    20USBHX2003825   -> 20USBHX2003825
    """
...
def read_pairs_csv(pairs_csv, institute="LBNL"):
    """
...
def make_ml_only_entry(ml_serial):
    """
    Keep ML serials that exist in lbnl_ml_serials.txt
...
def main():
    parser = argparse.ArgumentParser(
        description="Get LBNL HX Response Curve TC timestamps and pair HX with ML."
    )

    parser.add_argument(
        "--hx_file",
        default="serial_lists/lbnl_hx_serials.txt",
        help="Text file with one LBNL HX serial per line",
    )

    parser.add_argument(
        "--ml_file",
        default="serial_lists/lbnl_ml_serials.txt",
        help="Text file with one LBNL ML serial per line",
    )
    ...
    parser.add_argument(
        "--output_json",
        default="timestamps_list_lbnl.json",
        help="Output JSON file",
    )

    parser.add_argument(
        "--output_txt",
        default="formatted_timestamps_lbnl.txt",
    ...
    # Add ML-only rows if an ML exists in lbnl_ml_serials.txt
