...
Usage:
python plot_multi_inputnoise_noskip_UCSC.py -i 'UCSC/HX/SN20USBHX2003810/SN20USBHX2003810_*.json'
python plot_multi_inputnoise_noskip_UCSC.py -i 'UCSC/HX/SN*/*.json'

Output example:
UCSC/HX3/SN20USBHX2003810/inputnoise_noskip/SN20USBHX2003810-away.png
UCSC/HX3/SN20USBHX2003810/inputnoise_noskip/SN20USBHX2003810-under.png
'''
...
def get_module_names(files):
    ...
    for f in files:
        ...
        if module_name:
            modules.add(ensure_sn(module_name.replace("SN", "")))

    return sorted(modules)


def mk_inoise_plot(module_name, input_files, stream="under", output_base_dir="UCSC/HX3"):
    fig, ax = plt.subplots(figsize=(16, 9))

    module_name = ensure_sn(module_name.replace("SN", ""))
    ...
    # Saves into:
    # UCSC/HX3/SERIAL/inputnoise_noskip/
...
def main():
    ...
    parser.add_argument(
        "--serial_number",
        help="Serial number, e.g. 20USBHX2003810",
    )

    parser.add_argument(
        "-i",
        "--input",
        help="Glob pattern, e.g. 'UCSC/HX/SN20USBHX2003810/SN20USBHX2003810_*.json' or 'UCSC/HX/SN*/*.json'",
    )
    ...
    parser.add_argument(
        ...
        "--output",
        default="UCSC/HX3",
        help="Output base directory. Default: UCSC/HX3",
    )
    ...
    if args.input:
        ...
        input_files = sorted(
            glob(f"UCSC/HX/SN{serial_number}/SN{serial_number}_*.json")
        )
    ...
    # Important:
    # Do NOT infer UCSC/HX from the input path.
    # Always use args.output, which defaults to UCSC/HX3.
