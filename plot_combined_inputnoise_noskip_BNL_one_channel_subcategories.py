#!/usr/bin/env python3
"""
Script: plot_combined_inputnoise_noskip_BNL_one_channel_subcategories.py

Usage:
    python plot_combined_inputnoise_noskip_BNL_one_channel_subcategories.py -i "BNL/HX/SN*/*.json" -o "BNL/HX2"

    python plot_combined_inputnoise_noskip_BNL_one_channel_subcategories.py \
...
Behavior:
    ...
    * Problem PDFs are copied into:
          BNL/HX2/problem_inputnoise_noskip_histograms/
...
def write_error_summary_txt(results, output_base_dir, unreadable_files):
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL COMBINED INPUT-NOISE NOSKIP HISTOGRAM SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
        outfile.write(
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Plot combined BNL no-skip input-noise histograms for streams "
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Glob pattern, e.g. 'BNL/HX/SN*/*.json'",
    )
    ...
    if args.input:
        ...
        input_files = sorted(
            glob.glob(
                f"BNL/HX/SN{serial}/SN{serial}_*.json"
            )
        )
    ...
    print("\n" + "=" * 80)
    print("BNL COMBINED INPUT-NOISE NOSKIP PROBLEM HISTOGRAMS")
    print("=" * 80)
