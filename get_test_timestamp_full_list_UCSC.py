...
#   serial_lists/ucsc_ml_serials.txt
#   serial_lists/all_module_hybrid_pairs.csv
#
# Writes:
#   timestamps_list_ucsc.json
#   formatted_timestamps_ucsc.txt
#
# Run:
#   export ITK_DB_AUTH=YOUR_TOKEN
#
#   python get_test_timestamp_full_list_UCSCtest.py
# ============================================================
...
def add_sn_prefix(serial):
    """
    20USBHX2003578   -> SN20USBHX2003578
    SN20USBHX2003578 -> SN20USBHX2003578
    """
...
def remove_sn_prefix(serial):
    """
    SN20USBHX2003578 -> 20USBHX2003578
    20USBHX2003578   -> 20USBHX2003578
    """
...
# ============================================================
# Read serial text files
# ============================================================

def read_serial_file(serial_file, serial_type):
    """
    Read one serial per line.
...
def read_pairs_csv(pairs_csv, institute="UCSC"):
    """
    Read serial_lists/all_module_hybrid_pairs.csv.
    ...
    if not pairs_csv.exists():
        raise FileNotFoundError(
            ...
            f"    serial_lists/all_module_hybrid_pairs.csv"
        )
...
def make_ml_only_entry(ml_serial):
    """
    Keep ML serials that exist in ucsc_ml_serials.txt
    but do not have a matching HX in the CSV.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Get UCSC HX Response Curve TC timestamps and pair HX with ML."
    )

    parser.add_argument(
        "--hx_file",
        default="serial_lists/ucsc_hx_serials.txt",
        help="Text file with one UCSC HX serial per line",
    )

    parser.add_argument(
        "--ml_file",
        default="serial_lists/ucsc_ml_serials.txt",
        help="Text file with one UCSC ML serial per line",
    )
    ...
    parser.add_argument(
        "--output_json",
        default="timestamps_list_ucsc.json",
        help="Output JSON file",
    )

    parser.add_argument(
        "--output_txt",
        default="formatted_timestamps_ucsc.txt",
    ...
    # Add ML-only rows if an ML exists in ucsc_ml_serials.txt
