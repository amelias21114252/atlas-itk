...
Usage:
    ...
    python plot_combined_inputnoise_BNL_one_channel_subcategories.py \
...
Behavior:
    ...
    * Problem PDFs are copied into:
          BNL/HX/problem_inputnoise_histograms/
...
def write_error_summary_txt(results, output_base_dir, unreadable_files):
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL COMBINED INPUT-NOISE HISTOGRAM SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
        outfile.write(
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Plot combined BNL input-noise histograms for streams with "
    ...
    print("\n" + "=" * 80)
    print("BNL COMBINED INPUT-NOISE PROBLEM HISTOGRAMS")
    print("=" * 80)
