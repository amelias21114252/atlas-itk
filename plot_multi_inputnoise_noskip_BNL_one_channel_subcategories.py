...
Analyze BNL no-skip input-noise JSON files and save plots only for problem
module/stream combinations.

Run:
    python plot_multi_inputnoise_noskip_BNL_one_channel_subcategories.py -i "BNL/HX/SN*/*.json" -o "BNL/HX3"

Optional PDF-only run:
    python plot_multi_inputnoise_noskip_BNL_one_channel_subcategories.py \
        -i "BNL/HX/SN*/*.json" \
        -o "BNL/HX3" \
        --no_png
...
Plot behavior:
    ...
    * Each PDF is copied into every matching subcategory folder under:
          BNL/HX/problem_inputnoise_noskip_plots/
...
PROBLEM_PARENT_FOLDER = "problem_inputnoise_noskip_plots"
...
def plot_problem_module_stream(
    ...
    normal_dir = Path(output_base_dir) / module_name / "inputnoise_noskip"
...
def write_error_summary_txt(results, output_dir, unreadable_files):
    ...
    summary_path = output_dir / "inputnoise_noskip_error_summary_bnl.txt"
    ...
    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL INPUT NOISE NOSKIP ERROR SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def write_category_summary_txt(results, output_dir):
    ...
    summary_path = output_dir / "inputnoise_noskip_category_summary_bnl.txt"
    comments = build_module_comments(results)

    with summary_path.open("w") as outfile:
        outfile.write("=" * 80 + "\n")
        outfile.write("BNL INPUT NOISE NOSKIP CATEGORY SUMMARY\n")
        outfile.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Analyze both BNL no-skip input-noise streams and save only the "
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Glob pattern, for example 'BNL/HX/SN*/*.json'",
    )
    ...
    if args.input:
        ...
        input_files = sorted(
            glob(f"{args.output}/SN{serial}/SN{serial}_*.json")
        )

        if not input_files:
            input_files = sorted(
                glob(f"BNL/HX/SN{serial}/SN{serial}_*.json")
            )

        if not input_files:
            input_files = sorted(
                glob(f"SN{serial}/SN{serial}_*.json")
            )
    ...
    print("\n" + "=" * 80)
    print("BNL INPUT NOISE NOSKIP — ONE-CHANNEL PLOT TRIGGER")
    print("=" * 80)
    ...
    print(f"Normal plot folders: {output_dir}/SN20USBHX.../inputnoise_noskip/")
